...
public interface DeviceActivityRepository extends JpaRepository<DeviceActivity, Long> {
     ...
     * to skip filtering on that dimension. Returns all activities that have a
     * device serial reference (any actionable_type).
     */
    @Query(value = """
            ...
            WHERE a.user_id = :merchantId
              ...
              AND (CAST(:search AS VARCHAR) IS NULL OR (
                    ...
                    OR LOWER(COALESCE(a.actionable_type, '')) LIKE CAST(:search AS VARCHAR)
              ))
